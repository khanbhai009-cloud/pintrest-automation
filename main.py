...
def schedule_daily_pins():
    """
    Generate and register today's 20-pin schedule.
    Called once at startup and again via daily cron at 1:00 AM EST (= 10:30 AM India).
    Pins distributed randomly across EST 1:30 AM – 11:30 PM (India 3 PM – 11 AM).
    Gap between consecutive pins: random 20–30 minutes.
    """
    ...
    today        = now.date()
    window_start = _make_dt(today, 1, 30)    # EST 1:30 AM = India 3:00 PM
    window_end   = _make_dt(today, 23, 30)   # EST 11:30 PM = India 11:00 AM next day

    # If we're already past the window, schedule for tomorrow
    if now >= window_end:
        tomorrow     = today + timedelta(days=1)
        window_start = _make_dt(tomorrow, 1, 30)
        window_end   = _make_dt(tomorrow, 23, 30)

    WINDOW_MINUTES = 1320  # 1:30 AM → 11:30 PM = 22 hours
    MIN_GAP        = 20    # minimum gap (minutes) between any two consecutive pin slots
    MAX_GAP        = 30    # maximum gap — each gap is random between 20–30 min

    # ── Generate 20 time slots with random 20–30 min gap enforced ─────────────
    # Start from NOW (or window_start if future), not always from window_start
    # This ensures mid-day restarts still register future slots correctly
    slots = []
    elapsed = max(0, int((now - window_start).total_seconds() / 60))
    # First slot: 5–15 min from now (or from window start if not yet started)
    current = elapsed + random.randint(5, 15)
    for _ in range(20):
        if current >= WINDOW_MINUTES:
            break
    ...
    logger.info(
        ...
        f"   Window: {window_start.strftime('%I:%M %p')} EST → {window_end.strftime('%I:%M %p')} EST\n"
        f"   India Window: 3:00 PM → 11:00 AM (next day)\n"
...
async def lifespan(app: FastAPI):
    ...
    # Daily re-scheduler — fires at 1:00 AM EST (= 30 min before window opens, India 10:30 AM)
    scheduler.add_job(schedule_daily_pins, "cron", hour=1, minute=0, id="daily_scheduler")
